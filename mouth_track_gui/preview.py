...
def export_erase_range_preview_video(
    *,
    video: str,
    track_path: str,
    track_npz: str,
    ...
    try:
        ...
        log_fn(f"[info] 軽量見た目確認動画に使用する track: {track_path}")
        ...
        valid_idxs = np.where(track_data.valid[:n_out])[0]
        center_idx = int(valid_idxs[len(valid_idxs) // 2]) if len(valid_idxs) else min(max(n_out // 2, 0), max(n_out - 1, 0))
        offsets = [-15, -9, -3, 3, 9, 15]
        frame_idxs = [int(np.clip(center_idx + off, 0, max(n_out - 1, 0))) for off in offsets]
        frame_idxs = list(dict.fromkeys(frame_idxs)) or [center_idx]
        ...
        try:
            for frame_idx in frame_idxs:
                ...
                q = filled[frame_idx].astype(np.float32).reshape(4, 2)
                panels: list[np.ndarray] = []
                for pad_idx, pad_value in enumerate(pad_values):
                    q_pad = scale_quad_about_center(q, float(pad_value) / preview_pad_safe)
                    M = cv2.getPerspectiveTransform(src_pts, q_pad)
                    ...
                    cv2.putText(out, panel_info, (10, 28), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 0, 0), 3, cv2.LINE_AA)
                    cv2.putText(out, panel_info, (10, 28), cv2.FONT_HERSHEY_SIMPLEX, 0.75, panel_color, 1, cv2.LINE_AA)
                    if not bool(track_data.valid[frame_idx]):
                        cv2.putText(out, "INVALID (filled)", (10, 56), cv2.FONT_HERSHEY_SIMPLEX, 0.65, invalid_color, 2, cv2.LINE_AA)
                    panels.append(out)

                strip = np.concatenate(panels, axis=1)
                canvas = cv2.copyMakeBorder(strip, 120, 0, 0, 0, cv2.BORDER_CONSTANT, value=(18, 18, 18))
                info1 = f"frame {frame_idx+1}/{n_out}  cov={cov:.2f}  track={track_label}  red=erase  yellow=ring  inspect={inspect_boost:.1f}"
                info2 = "macOS fallback preview clip (non-interactive). Use ② 口消し動画生成 for final verification."
                info_rows = [(24, info1), (52, info2)]
                if color_adjust is not None:
                    info_rows.append((80, f"mouth adj  bri={color_adjust.brightness:.0f} sat={color_adjust.saturation:.2f} warm={color_adjust.warmth:.0f} edge={color_adjust.edge_priority:.2f}"))
                for y, text in info_rows:
                ...
                if writer is None:
                    ...
                    writer = cv2.VideoWriter(out_path, fourcc, 6.0, (display.shape[1], display.shape[0]))
                    if not writer.isOpened():
                        writer.release()
                        writer = None
                        show_error("エラー", f"プレビュー動画を書き出せませんでした: {out_path}")
                        return None
        ...
        if not os.path.isfile(out_path):
            show_error("エラー", f"プレビュー動画を書き出せませんでした: {out_path}")
            return None
        log_fn(f"[info] 軽量見た目確認動画を書き出しました: {out_path}")
        return out_path
    finally:
        try:
            cap.release()
